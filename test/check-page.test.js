...
//
// check-page.js is `(() => { ... })();` and evaluates to a boolean: true means
// "this is an instance-level failure, fall away from it", false means "leave it
// alone". It reads only `document` and the `HTMLElement` global, so we wrap the
// source in a Function that takes both.
...
test("Nitter error panel naming a rate limit is a failure", () => {
    assert.equal(classifyHTML("error-panel-rate-limited.html"), true);
});
...
test("fork rate-limit page without .error-panel is a failure", () => {
    assert.equal(classifyHTML("fork-rate-limit-no-panel.html"), true);
});

test("operator shutdown page (no Nitter markers) is a failure", () => {
    assert.equal(classifyHTML("shutdown-page.html"), true);
});
